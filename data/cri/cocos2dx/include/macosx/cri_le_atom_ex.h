...
/* バージョン情報 */
/* Version Number */
#define CRIATOMEX_VERSION				(0x02100000)
#define CRIATOMEX_VER_NUM				"2.10.00"
 ...
 * \param[out]	p_config	初期化用コンフィグ構造体へのポインタ
 * \par 説明:
 * ACFコンフィグ構造体（ ::CriAtomExConfig ）に、デフォルトの値をセットします。
 * \sa
 * CriAtomExAcfConfig
 */
 ...
 * ::criAtomExPlayer_ChangeDefaultPanSpeakerType関数、::criAtomExPlayer_SetPanSpeakerType 関数で利用します。<br>
 * \par 備考:
...
typedef struct CriAtomExHcaMxConfigTag {
	...
	*/
	...
	/*JP
		...
		\par 備考:
		例えばHCA-MX再生時に::criAtomHcaMx_SetFrequencyRatio 関数に 2.0f を指定してピッチを上げる場合は、
 ...
 * 本関数でマスタタイマを一時停止しておくことで、休止中のシーケンスの進行を止める事が出来ます。<br>
 * \attention
 * 本関数で一時停止する対象はあくまでAtomライブラリ内のマスタタイマです。<br>
 * 本関数では発音中のボイス等を一時停止出来ません。
 * 本関数で設定したポーズフラグは、CRI Atomサーバ処理が実行されたタイミングで反映されます。<br>
 * 即座に同期をとる必要がある場合は、::criAtomEx_ExecuteAudioProcess関数を呼び出す事で同期をとることができます。<br>
 ...
 * - AISACコントロール値（::criAtomExPlayer_SetAisacControlById関数、::criAtomExPlayer_SetAisacControlByName 関数で設定）
 ...
 * ::criAtomExPlayer_SetPanSpeakerType関数を実行していないAtomExプレーヤは、全て本関数で設定した出力スピーカータイプで再生されます。<br>
 * \par 備考:
 * 本関数の設定はパン3Dと3Dポジショニングにおけるパンニング計算に影響します。<br>
 * ライブラリ初期化時のデフォルト値は4chパンニング（::CRIATOMEX_PAN_SPEAKER_TYPE_4CH）です。<br>
 ...
 * ラベル情報を含むすべてのプレーヤ設定値削除は ::criAtomExPlayer_ResetParameters 関数を実行してください。<br>
 ...
 * 	- 最大距離：0.0f
 * 	.
 * デフォルト値は、::criAtomEx3dSource_ChangeDefaultMinMaxDistance 関数にて変更可能です。<br>
