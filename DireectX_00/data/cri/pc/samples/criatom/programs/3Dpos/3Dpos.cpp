...
static App3dPositioningPreviewItem g_preview_list[] = {
	...
	{CRI_3DPOS_FOOTSTEP_LOOP,	APP_MOTION_GO_AROUND_LISTENER,        "Go around Listener"},
...
static CriBool app_atomex_initialize(AppObj *app_obj)
{
	...
	/* DSP設定のアタッチ */
	criAtomEx_AttachDspBusSetting(CRI_ATOMCRAFTWORK_SAMPLES_ACF_DSPSETTING_DSPBUSSETTING_0, NULL, 0);
...
void app_update_source(AppObj *app_obj)
{
	switch (g_preview_list[app_obj->current_preview_index].motion_pattern) {
		...
		case	APP_MOTION_GO_AROUND_LISTENER:
			/* リスナー(原点)の周囲を回る */
			{
				CriFloat32 rr = pow(app_obj->source_pos.x, 2.0f) + pow(app_obj->source_pos.z, 2.0f);
				CriFloat32 acceleration = GM / rr;
				CriFloat32 r = sqrt(rr);
