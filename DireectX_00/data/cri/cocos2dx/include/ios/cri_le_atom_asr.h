 ...
 * デフォルトのバス数です。
 */
#define CRIATOMEXASR_DEFAULT_NUM_BASES	(8)
 ...
 * ASRが処理可能な最大バス数です。
 */
#define CRIATOMEXASR_MAX_BASES			(64)
...
{\
	...
	(p_config)->num_buses = CRIATOMEXASR_DEFAULT_NUM_BASES; \
...
typedef struct CriAtomExAsrConfigTag {
	...
	*/
	...
	/*JP
		...
		バスはサウンドのミックスや、DSPエフェクトの管理等を行います。<br>
	*/
	CriSint32 num_buses;
...
typedef enum {
	...
	CRIATOMEXASR_DSP_ID_COMPRESSOR_EX,		/*JP< コンプレッサ(拡張)	*/
 ...
 * \ingroup ATOMEXLIB_ASR
 * \par 説明:
 * 分割EQのタイプを指定するためのデータ型です。<br>
 * ::CriAtomExAsrCompressorExParameter 関数で利用します。
 */
typedef enum {
	CRIATOMEXASR_SPLIT_EQ_NONE     = 0,		/*JP<SplitEQ無効        */
	CRIATOMEXASR_SPLIT_EQ_LOWPASS  = 1,		/*JP<ローパスタイプ		*/
	CRIATOMEXASR_SPLIT_EQ_HIGHPASS = 2,		/*JP<ハイパスタイプ		*/
	CRIATOMEXASR_SPLIT_EQ_BANDPASS = 3,		/*JP<バンドパスタイプ	*/
	CRIATOMEXASR_SPLIT_EQ_NOTCH    = 4,		/*JP<ノッチタイプ		*/
...
/*JP
 * \brief コンプレッサの拡張パラメータ
 * \ingroup ATOMEXLIB_ASR
 * \par 説明:
 * コンプレッサDSPのパラメータを設定するための構造体です。<br>
 * 本構造体を使用するとEQパラメータで指定した帯域のみコンプレッサをかけることができます。<br>
 * \sa criAtomExAcf_GetDspFxParameters, criAtomExAsr_SetDspParameter
 */
