...
{
	//文字フォントの設定
	LPCSTR Font = _T("tahoma");
	if ((D3DXCreateFont(CDirectxMgr::Getintance().GetDxDevice(), 14, 7, FW_BOLD, NULL, FALSE, SHIFTJIS_CHARSET, OUT_DEFAULT_PRECIS, PROOF_QUALITY, FIXED_PITCH | FF_MODERN, Font, &pFont)))
	{
		return;
	}
}
...
void CDebug::Release()
{
	Complete_type_safe_delete(pFont);
}